...
const context={SITE_THEME:'noel',CHRISTMAS_POOL,BUZZ,THEMES,FAF,DISCOVERY:{},RULES:{MAX_JOUEURS:8},ls:{set(){}},db:{},uid:()=> 'test',doc:()=>({}),getDoc:async()=>({exists:()=>true,data:()=>({premium:paid})}),console};
...
const standardContext={...context,SITE_THEME:'standard'};
vm.runInNewContext(source,standardContext);
assert.equal(standardContext.testPlan.guard('event','noel').ok,false);
assert.throws(()=>standardContext.testPlan.pools('noel'));
